...
-- Kompatibilitäts-Schicht: sorge dafür, dass sowohl `Config.Keybinds` (friendly labels/keys)
-- als auch `Config.KeyBindings` (numerische Keycodes) vorhanden und synchron sind.
do
    ...
    for k, v in pairs(defaults) do
        Config.Keybinds[k] = Config.Keybinds[k] or v
    end

    -- Falls numerische KeyBindings fehlen, fülle sie aus vorhandenen MenuKey-Variablen oder belasse Defaults
    Config.KeyBindings = Config.KeyBindings or {}
    Config.KeyBindings.menu = Config.KeyBindings.menu or Config.MenuKey or 244
    Config.KeyBindings.bossMenu = Config.KeyBindings.bossMenu or Config.BossMenuKey or 244
    Config.KeyBindings.bossMenuModifier = Config.KeyBindings.bossMenuModifier or Config.BossMenuModifier or 21
    Config.KeyBindings.toggleHUD = Config.KeyBindings.toggleHUD or Config.ToggleHUDKey or 20
    Config.KeyBindings.smoke = Config.KeyBindings.smoke or Config.SmokeKey or 38

    -- Versuche, friendly `key` Strings aus numerischen KeyBindings zu befüllen (nur als string-representation)
    for k, field in pairs({menu = 'menu', bossMenu = 'bossMenu', toggleHUD = 'toggleHUD', smoke = 'smoke'}) do
        if Config.KeyBindings[field] and (not Config.Keybinds[field] or not Config.Keybinds[field].key) then
            Config.Keybinds[field] = Config.Keybinds[field] or {}
            Config.Keybinds[field].key = tostring(Config.KeyBindings[field])
        end
    end
end
